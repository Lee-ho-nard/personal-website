...
**Role:** Keeps the hero orb present as an ambient anchor while scrolling, instead of scrolling away with the hero

On Home only, and only absent reduced motion, `.hero .sphere` switches to `position: fixed`, anchored near the top-right of the viewport at the same resting spot the absolute version occupies at scroll 0 (so there's no jump at load). A `scroll`/`resize` listener (rAF-throttled, never run directly off the raw event) tracks scroll progress against the hero's height and writes `--scroll-scale` (recedes from 1 toward ~0.65) and `--scroll-y` (a slight upward drift) as the user scrolls past the hero, composed into the same `transform` as the cursor-follow offset so both coexist. Over the same span, `--scroll-x` measures the actual card's right edge each frame and pushes the orb just far enough right to clear it — a measured guarantee against ever overlapping a card, not an assumed viewport-margin formula, since the safe margin varies with viewport width (it's 0 on anything at or under the page's 1400px max-width, in which case the orb ends up pushed mostly off-screen rather than pretending there's room). Past a point just before the footer, the same loop fades `--scroll-opacity` to 0 so the orb never overlaps the Contact section either. Work/About/Life never opt into any of this — their `.sphere` stays `position: absolute` in the page-header, static regardless of scroll. Under reduced motion, `.hero .sphere` falls back to the plain absolute, non-fixed rule — no scroll-tracking transform at all.
