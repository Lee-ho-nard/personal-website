...
| orb blob | 42s | `--duration-orb-blob` | Ambient loop — the sphere's slow, subtle silhouette morph (see `orb-blob-morph`). Deliberately not a clean multiple/divisor of `--duration-orb` so the two never fall into a visibly synced cycle |
