...
Two more layers — a soft ambient glow and sparse drifting particles — sit behind the orb, deliberately independent of everything above: both are built and positioned in their own dedicated section of motion.js, read `.sphere`'s own live *computed* transform each tick rather than sharing or re-deriving any of the five systems' internal state, and never touch the SVG filter/displacement graph at all. `.orb-glow` is a separate blurred `<div>` (`filter: blur(40px)`, `background: var(--gradient-iridescent-sphere)`), not a `drop-shadow()` chained onto `.sphere` itself — chaining another filter operation onto the same element already carrying the liquid filter would compound that (comparatively expensive) cost on every repaint, where a second, independent element's plain `blur()` stays cheap and isolated. It approximates the orb's current hue rather than sampling it directly: a CSS `@keyframes` animation rotates the glow's own `hue-rotate()` through 360° over the same `--duration-orb` length as the orb's own SMIL hue rotation, so the two stay visually in sync without the glow reading any actual pixel value. `.orb-particles` is a `<canvas>` (deliberately not SVG — a couple of small radial-gradient circles redrawn per frame is far cheaper than another `feTurbulence`/`feDisplacementMap` graph), sized 1.5× `.sphere`'s own box so drifting particles have real room to fade out in rather than being clipped by the canvas's own hard edge. At most two particles are ever active at once, spawning at a random point around the orb's own edge every 3–8s, drifting slowly up/outward, and fading in/out over a 5–8s lifespan; each samples one of the gradient's three chromatic stops (yellow/pink/blue — white has no hue to rotate) and applies the same `--duration-orb`-length rotation the glow uses, computed directly in JS since canvas fill colors need a real value, not a CSS animation. Measured cost of both ticks' own work (position-copy plus a full two-particle redraw) came out to roughly 0.02ms combined — under 0.15% of a 16.7ms frame budget — nowhere near the liquid filter's own measured cost; that's what "genuinely cheap, standard techniques" concretely means here. `.orb-glow` stays visible under reduced motion (same base position as `.sphere`, just without the hue-rotate animation or any JS-driven position tracking, since `.sphere` itself is static then too); `.orb-particles` is never created at all in that case, matching how the liquid filter's own JS setup is skipped entirely under reduced motion.
