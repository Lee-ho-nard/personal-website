...
Large circular form (roughly 50% of viewport height), same `.sphere` element and exact size on every page — only its position differs: bled above/right of the Home hero (anchored to the same center point as the concentric rings — see Concentric Circle Ornament), sitting flush inside the page column on Work/About/Life. `.page-header` is sized tall enough (`min-height: 360px`) to hold the full circle with no `overflow: hidden` anywhere, so the orb is never cropped on any page. Filled with the iridescent gradient (yellow → pink → blue → white at 255deg). No border, no shadow. Deliberately carries no `view-transition-name` — during page-to-page navigation it's just ordinary root content, carried along in the same directional slide as everything else (see Directional Page Swipe) rather than travelling as its own independently-interpolated element. Outside of a transition, it carries three simultaneous, independent motion layers, all optional and independently gated: a barely-perceptible ambient hue drift (`--duration-orb`, linear, continuous, on unless reduced motion); on fine-pointer devices only, a restrained cursor-follow drift (`--orb-follow-distance`, `--orb-follow-lerp`) that trails the pointer rather than snapping to it, on whichever page the cursor is over; and, Home only, a scroll-linked persistence (see Scroll-Linked Orb). Never a spin, never a hard follow, never anything attention-grabbing.

### Scroll-Linked Orb (Home only)
**Role:** Keeps the hero orb present and genuinely roaming as an ambient anchor while scrolling, instead of scrolling away with the hero or being boxed into a thin lane

On Home only, and only absent reduced motion, `.hero .sphere` switches to `position: fixed`, anchored near the top-right of the viewport at the same resting spot the absolute version occupies at scroll 0 (so there's no jump at load) — precisely centered on the same point as the concentric rings, verified via `getBoundingClientRect()` against the live rings rather than assumed, since `.page`'s own max-width centering (past 1400px + padding) shifts the hero-relative rings' viewport position in a way a flat px offset on the viewport-relative fixed orb can't track; the `right` offset mirrors that centering math via `calc()` instead. Now/Contact carry no card box (see `#now, #contact`), so there's real open ground to roam: a `scroll`/`resize` listener (rAF-throttled, never run directly off the raw event) tracks scroll progress against the hero's height for `--scroll-scale` (recedes from 1 toward ~0.65) and `--scroll-y` (a slight upward drift), and separately tracks progress across the *whole* page for `--scroll-x` — a sine arc that drifts the orb left toward the page's midpoint and back, its amplitude measured each frame against the actual rendered edge of the Now/Contact prose text (via `Range.getClientRects()`, since a paragraph's own box reports its max-width cap or container edge, not where the visible glyphs really end) so it's real open space at any viewport width, never an assumed fraction of it. Past a point just before the footer, the same loop fades `--scroll-opacity` to 0. Work/About/Life never opt into any of this — their `.sphere` stays `position: absolute` in the page-header, static regardless of scroll. Under reduced motion, `.hero .sphere` falls back to the plain absolute, non-fixed rule — no scroll-tracking transform at all.

### Directional Page Swipe
**Role:** Carries the whole page — orb included — as one unified slide between tabs, rather than a directionless cross-fade or a separately-travelling orb

Site-wide `@view-transition { navigation: auto; }` opts every page into cross-document view transitions. On `pageswap`, motion.js compares the outgoing and incoming page against the nav order (Home → Work → About → Life) and tags the transition via `event.viewTransition.types.add("forward" | "back")` — no direction, no tag, and a plain cross-fade applies instead. `html:active-view-transition-type(forward)` slides `::view-transition-old(root)` left and out while `::view-transition-new(root)` slides in from the right; `back` reverses both, both on `--duration-base`/`--ease-standard`. Because the orb carries no `view-transition-name` of its own (see Hero Gradient Sphere), it's simply part of these `root` snapshots — it moves in the exact same transform as the rest of that page's content, reading as attached to the page rather than as an independently-interpolated travelling element. The moment the new page has landed, the transition is over and the orb resumes that page's own normal behavior (hue-drift, cursor-follow, and on Home the scroll-linked persistence) — the "carried along" behavior only exists for the duration of the swap itself. Unsupported browsers ignore the at-rule entirely and fall back to a normal, instant navigation — no feature-detection script needed, since that fallback is CSS's own forward-compatible parsing behavior. `prefers-reduced-motion: reduce` collapses the slide to a near-instant cut.
