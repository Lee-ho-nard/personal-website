...
Every page's `.sphere` — identical size everywhere — shares `view-transition-name: hero-orb`. Site-wide `@view-transition { navigation: auto; }` opts every page into cross-document view transitions, so the browser — not JS — morphs the orb's position automatically when navigating via the nav bar (a pure move, since size never changes), while the rest of the page content transitions using `::view-transition-old(root)`/`::view-transition-new(root)`. The orb's group animation uses `--duration-slow`/`--ease-standard`; the root transition uses `--duration-base`/`--ease-standard`, and slides directionally (see Directional Page Swipe) rather than plain-crossfading when a direction is known. Unsupported browsers ignore the at-rule and the property entirely and fall back to a normal, instant navigation — no feature-detection script needed, since that fallback is CSS's own forward-compatible parsing behavior. `prefers-reduced-motion: reduce` collapses every view-transition animation to near-instant.

### Directional Page Swipe
**Role:** Gives the root content transition a sense of place within the site's nav order (Home → Work → About → Life), rather than a directionless cross-fade

On `pageswap`, motion.js compares the outgoing and incoming page against the nav order and tags the transition via `event.viewTransition.types.add("forward" | "back")` — no direction, no tag, and the plain cross-fade above applies. `html:active-view-transition-type(forward)` slides `::view-transition-old(root)` left and out while `::view-transition-new(root)` slides in from the right; `back` reverses both. Both run on `--duration-base`/`--ease-standard`, entirely independent of the orb's own `::view-transition-group(hero-orb)` animation, which keeps the browser's default translate interpolation throughout.
