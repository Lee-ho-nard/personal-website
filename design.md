...
| band transition | 350px | *(inline in motion.js, `BAND_TRANSITION`)* | Real vertical gap over which a `.zigzag` element's contribution to the safety bounds fades in, instead of switching on the instant it crosses the band edge — kept under half of `--zigzag-gap` so two adjacent elements' fades can't overlap and manufacture a new infeasible scrollY range |
...
Each `.zigzag` element's own contribution to that band-membership set fades in over `BAND_TRANSITION` (350px of real vertical gap between the element and the orb's band), rather than switching on the instant its rect crosses the band edge. This was a real, reported bug, not a preemptive smoothing pass: with a hard on/off cutoff, `boundedLower`/`boundedUpper` could swing by hundreds of px across a single small scroll step wherever an element's rect crossed that edge — invisible scrolling down (the deliberate position starts centered and simply lerps outward as the bound loosens, well within the old bound the whole time) but a visible snap scrolling up through the same point, since the position had drifted out under the wide-bounds regime and the hard safety clamp — which must apply instantly, not lerp, to guarantee the render is never unsafe — then had to correct it hundreds of px in a single frame. `BAND_TRANSITION` is deliberately kept under half of `--zigzag-gap` (350 of 740px): wider values measurably smoothed the swing further, but far enough that two adjacent elements' transition zones started overlapping, manufacturing brand-new scrollY ranges where neither text bound could be satisfied at once — the existing infeasible-fallback (below) masks that safely, but leaning on it more than the original design did isn't worth trading away for extra smoothness. The fade uses a large-but-finite sentinel (`BAND_SENTINEL`) instead of literal `Infinity` for an element that isn't constraining — `lowerBase`/`upperBase` being real `Infinity` would flip `isFinite()` checks downstream at the exact moment an element (de)activates, which just relocates the same discontinuity — and a steep `t⁴` falloff (`BAND_FALLOFF_POWER`) that stays within a percent or two of fully active for most of the transition and only drops sharply right at the boundary, so a still-fairly-close element doesn't get swamped by that large a correction too early.
