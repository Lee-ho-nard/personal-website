...
Large circular form (roughly 50% of viewport height), same `.sphere` element and exact size on every page — only its frame differs: full-bleed in the Home hero, cropped inside the smaller Work/About/Life page-header (`.page-header { overflow: hidden }`), so the shared `view-transition-name` interpolates as a pure move, never a resize. Filled with the iridescent gradient (yellow → pink → blue → white at 255deg). No border, no shadow. Carries three simultaneous, independent motion layers, all optional and independently gated: a barely-perceptible ambient hue drift (`--duration-orb`, linear, continuous, on unless reduced motion); on fine-pointer devices only, a restrained cursor-follow drift (`--orb-follow-distance`, `--orb-follow-lerp`) that trails the pointer rather than snapping to it, on whichever page the cursor is over; and, Home only, a scroll-linked persistence (see Scroll-Linked Orb). Never a spin, never a hard follow, never anything attention-grabbing.
...
On Home only, and only absent reduced motion, `.hero .sphere` switches to `position: fixed`, anchored near the top-right of the viewport. A `scroll` listener (rAF-throttled, never run directly off the raw event) tracks scroll progress against the hero's height and writes `--scroll-scale` (recedes from 1 toward ~0.65) and `--scroll-y` (a slight upward drift) as the user scrolls past the hero, composed into the same `transform` as the cursor-follow offset so both coexist. Past a point just before the footer, the same loop fades `--scroll-opacity` to 0 so the orb never overlaps the Contact section. Work/About/Life never opt into this — their `.sphere` stays `position: absolute` in the page-header, static regardless of scroll. Under reduced motion, `.hero .sphere` falls back to the plain absolute, non-fixed rule — no scroll-tracking transform at all.
