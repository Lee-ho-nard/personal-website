...
- **Card padding:** 30px
- **Element gap:** 19px
- **Zigzag gap:** 700px (`--zigzag-gap`) — the gap before every `.zigzag` section after the first. Well above the normal 76–119px section-gap range, but not for the weave's sake (the S-curve adapts to whatever room it's given). Re-derived (not nudged) after `BAND_TRANSITION` (see Scroll-Linked Orb) was added: a fully rigorous, always-comfortable-margin minimum came out viewport-relative, and at typical desktop widths, no smaller than the old 740px — About (most zigzag sections, so its later transitions land at a higher growthProgress than Home's ever do) is the binding page. This flat, tighter value is a deliberate tradeoff, chosen over that fully rigorous one: it visibly tightens section pacing everywhere, in exchange for a brief (~30px of scroll, ~1024px width), safely-handled exception on About where the existing infeasible-fallback solver has to pin the orb to a fixed point for a moment instead of gliding through — verified via `getBoundingClientRect()` sweeps at 700/1024/1700px, not assumed (see Scroll-Linked Orb).
...
| base growth plateau | 1.65 | *(inline in motion.js, `BASE_GROWTH_PLATEAU`)* | The orb's depth-based base size (independent of breathing) once fully grown — starts at 1 at scroll 0 and grows smoothly and monotonically from there (no recede dip; see Scroll-Linked Orb) to this noticeably larger resting size. Combined with breathe amplitude at its widest-open moments, this is what gets the orb to roughly a third of the viewport width at its largest |
...
| min orb half-width fraction | 0.2 | *(inline in motion.js, `MIN_ORB_HALF_WIDTH`)* | Floor on the solved half-width, as a fraction of the orb's own current desired size (`orbHalfWidthSafe`) — not a flat epsilon. A page with a lone zigzag element (no opposite-side neighbor to average against) near full depth-growth in a mid-width viewport could otherwise shrink toward true zero (measured ratio ~0.01 on About at 1024px) while still reading as "feasible," since a small-enough orb can always fit somewhere; a real floor makes a spot that tight fall through to the infeasible-fallback position instead. 0.2 specifically sits right at the point past which raising it stops changing the worst case at all (a separate, floor-independent constraint is already binding there) and only multiplies how often that fallback fires |
| band transition | 350px | *(inline in motion.js, `BAND_TRANSITION`)* | Real vertical gap over which a `.zigzag` element's contribution to the safety bounds fades in, instead of switching on the instant it crosses the band edge — exactly half of `--zigzag-gap` (700px). Two adjacent elements' fades touching at the exact midpoint of the gap is by design now (see `--zigzag-gap`'s own comment): a deliberate, bounded, rare exception on About, not an oversight |
| band sentinel | 4000 | *(inline in motion.js, `BAND_SENTINEL`)* | Stand-in for "this element doesn't constrain the orb," used instead of literal `Infinity` so `isFinite()` checks downstream never flip (which would just relocate the same discontinuity the band transition fade exists to remove) |
| band falloff power | 4 | *(inline in motion.js, `BAND_FALLOFF_POWER`)* | Shape of the band-transition fade (`t⁴`) — stays within a percent or two of fully active for most of the transition and only drops sharply right at the boundary, so a still-close element isn't swamped by the sentinel-scale correction too early |
...
On every page, and only absent reduced motion, `.orb-frame .sphere` switches to `position: fixed`, anchored at a resting spot set per context via `--orb-rest-top`/`--orb-rest-right` (declared on `.hero` and `.page-header` respectively) — Home's ring-aligned bleed, Work/About/Life's flush page-column spot — reproducing each context's own absolute-mode position at scroll 0 (so there's no jump at load), and using `--page-edge-right` (a shared `calc()` mirroring `.page`'s own max-width centering math) rather than a flat px offset, since a static value drifts out of alignment once the viewport exceeds 1400px + padding. A continuous `requestAnimationFrame` tick (running every frame while unreduced, the same pattern as the cursor-follow orb, rather than only reacting to `scroll`/`resize` events) tracks progress against the hero/page-header's own height for `--scroll-y` (a slight upward drift) and separately drives `--scroll-x` — the S-curve weave.

`--scroll-scale`'s base (`depthBase`) starts at the normal size (1) at scroll 0 and grows smoothly and monotonically from there, leveling off at `BASE_GROWTH_PLATEAU` (1.65) at `GROWTH_LEVEL_FRACTION` (0.6) of the way from the header's end to the page's own last real content — measured per page, not a fixed pixel distance, so every page levels off well before its own footer regardless of how long its content runs. An earlier version started this ramp only after the header (`growthStart` = the header's own height), using the header-recede curve as growth's own starting point instead of a flat 1 — since that recede curve dropped to 0.65 across that same span and growth hadn't begun accumulating by the time it bottomed out, the orb visibly shrank before it grew, right at the top of every page. Ramping across the *entire* 0..`growthLevelAt` span instead of `growthStart`..`growthLevelAt` removed the recede as an input to size entirely, so there's nothing left to dip before growth takes over. Breathing (below) still applies on top of this depth-based base, not instead of it, so the orb's size at any instant is "how deep down the page" times "how much open space is nearby" times "the current organic scale wobble" — three independent signals compounding, not one.
...
Each `.zigzag` element's own contribution to that band-membership set fades in over `BAND_TRANSITION` (350px of real vertical gap between the element and the orb's band), rather than switching on the instant its rect crosses the band edge. This was a real, reported bug, not a preemptive smoothing pass: with a hard on/off cutoff, `boundedLower`/`boundedUpper` could swing by hundreds of px across a single small scroll step wherever an element's rect crossed that edge — invisible scrolling down (the deliberate position starts centered and simply lerps outward as the bound loosens, well within the old bound the whole time) but a visible snap scrolling up through the same point, since the position had drifted out under the wide-bounds regime and the hard safety clamp — which must apply instantly, not lerp, to guarantee the render is never unsafe — then had to correct it hundreds of px in a single frame. `BAND_TRANSITION` is exactly half of `--zigzag-gap` (350 of 700px): the two values were re-derived together — `--zigzag-gap` was deliberately tightened past the point that keeps every transition zone-pair fully separated (see its own comment in the Layout section), so on About specifically, two adjacent elements' fades can now touch right at the gap's midpoint. The existing infeasible-fallback (below) masks that safely; it's a deliberate, bounded, rare tradeoff for visibly tighter section pacing everywhere, not an oversight. The fade uses a large-but-finite sentinel (`BAND_SENTINEL`) instead of literal `Infinity` for an element that isn't constraining — `lowerBase`/`upperBase` being real `Infinity` would flip `isFinite()` checks downstream at the exact moment an element (de)activates, which just relocates the same discontinuity — and a steep `t⁴` falloff (`BAND_FALLOFF_POWER`) that stays within a percent or two of fully active for most of the transition and only drops sharply right at the boundary, so a still-fairly-close element doesn't get swamped by that large a correction too early.

Given that set, the solver finds the largest half-width consistent with every active bound at once: clearing a left-aligned element's margin, clearing a right-aligned one's, keeping both simultaneously satisfiable if both are active (rare — see the `--zigzag-gap` note above), and staying within `VIEWPORT_BLEED_FRACTION` of the viewport's own edges (a text-safety bound can still demand more room than even a generous bleed allows once a paragraph is wide relative to a narrow window — an orb that's clear of the text but invisible off-screen isn't actually safe either). That solved half-width becomes both the bound used to clamp `--scroll-x` and a cap on the actual rendered size (see the breathing paragraph below) — what's on screen always matches what the safety math assumed, rather than the render quietly exceeding it. Solving for "the largest half-width consistent with every bound" only guarantees *some* size fits — nothing stopped that size from being arbitrarily small until `MIN_ORB_HALF_WIDTH` (see the token table) put a real floor under it, at a fraction of the orb's own current desired size rather than a flat epsilon. A lone zigzag element (no opposite-side neighbor to average its bound against) near full depth-growth in a mid-width viewport could otherwise shrink the render toward true invisibility — measured directly, not assumed: ratio as low as ~0.01 of the intended size on About at 1024px — while every check upstream still read it as perfectly feasible, since a small-enough orb can always find room. If even the smallest *sane* half-width (the real floor, not the epsilon) can't satisfy every bound — content genuinely too tight for any acceptable orb size, which does happen at some narrow-viewport/deep-scroll combinations, more often now that the floor itself can trigger it — the solver gives up on the viewport-bleed preference first and targets the active text bound(s) exactly; violating "stay comfortably on screen" is the acceptable failure mode here, violating "never overlap text" is not. In the most extreme cases this can push the orb's position past the viewport edge entirely rather than the more usual partial bleed — still correct: a text-safety guarantee that quietly stopped at the viewport edge wouldn't actually be one.

This whole solve is intentionally never faded by *scroll position* the way the preference above is — the per-element `BAND_TRANSITION` fade above is a function of real measured gap distance, not of a scroll fraction, so it's still a measurement of where the orb's real, currently-rendered band physically is, not a scroll-position-based approximation of it; faking it out on scroll position alone would mean claiming safety it doesn't actually have. An earlier version *did* fade both together (sharing one `progress` value tied to the header's height), which is exactly what caused a reported backtrack in the S-curve — once a section's text got close enough to need clamping, the clamp would yank the still-fading-in preference straight to full strength in a single frame. Decoupling them, with the short fixed `WEAVE_INTRO_DISTANCE` above standing in for the old fade, is what fixed it.
...
The same weave value also drives a "breathing" resize: near its natural resting center (small weave offset) the orb reads as being in open space and grows; leaning hard toward a text block, it shrinks — a swing meant to be clearly visible while scrolling at a normal pace, not just on a before/after screenshot. `openness` (how "in open space" the orb currently reads) is the lean distance normalized against the orb's own current diameter (`baseWidth × depthBase`), not a flat px constant — it used to be a flat 220px, tuned back when the orb's own diameter was roughly that size, and once depth-growth and viewport-relative sizing made the real diameter several times bigger without this threshold following, almost any real safety-driven lean saturated `openness` to 0 everywhere content was nearby, which visually canceled the depth growth out almost entirely (a shipped regression, fixed after the fact once real-DOM measurement caught it — see below). It's tied to the same value driving the S-curve and lerped by the same `WEAVE_LERP`, so size and position move and settle as one motion rather than two competing effects. Because that resize is computed *from* the weave position, which is itself computed *against* the orb's own half-width, the edge-safety math throughout uses a conservative, worst-case-breathed half-width (`× 1.39` — `BREATHE_SAFETY_FACTOR`, i.e. 1 + breathe amplitude + organic scale headroom — rather than resolving the circular dependency exactly), so the real, possibly-smaller size is always at least as safe. When the solver above has had to shrink the orb to fit a tight spot, that same lerp additionally caps the breathing target so the actual render never exceeds the solved half-width either — a visible "squeeze through the gap" that settles back out once the constraint clears.

Even with `openness` scaled to the orb's own size, clearing the last section's text right before the footer can still take a lean of a couple orb-diameters at this scale — an inherent consequence of the orb being this big, not really the "hugging a text edge" case the shrink side was designed for. Left alone, that still saturates `openness` to 0 exactly where `growthProgress` has plateaued, i.e. exactly where the orb is supposed to read as biggest, multiplying the depth growth by breathing's bare floor (0.65×) and erasing most of it. The shrink floor is therefore raised toward 1 as `growthProgress` approaches its plateau (`breatheFloor = 1 - BREATHE_AMPLITUDE × (1 - growthProgress)`, `rawBreathe = breatheFloor + openness × (breatheCeil - breatheFloor)`) — a page just past its header (`growthProgress` 0) still gets the full ±35% swing, exactly reproducing the original formula, while a page's final stretch can no longer have its growth multiplied away by a forced lean. The expand side keeps its full range throughout, so breathing still reads as a lively pulse even at the largest sizes.
